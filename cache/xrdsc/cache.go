...
func StatsEnabled(enable bool) Option {
	return func(opt *Options) {
		opt.StatsEnabled = enable
	}
}
func Marshal(f MarshalFunc) Option {
	return func(opt *Options) {
		opt.Marshal = f
	}
}
func Unmarshal(f UnmarshalFunc) Option {
	return func(opt *Options) {
		opt.Unmarshal = f
	}
}
...
func New(redis *xredis.Redis, opt ...Option) *Cache {
	...
	if cacher.opt.Marshal == nil {
		cacher.marshal = cacher._marshal
	} else {
		cacher.marshal = cacher.opt.Marshal
	}

	if cacher.opt.Unmarshal == nil {
		cacher.unmarshal = cacher._unmarshal
	} else {
		cacher.unmarshal = cacher.opt.Unmarshal
	}
	return cacher
}
